          ...
          return applyToWindow(function (index: number, offset: number) {
            ...
            const spoofFunctions: { [key: string]: Function } = {
              "window.navigator.webdriver": () =>
                fake_window.navigator.webdriver,
              "window.navigator.userAgent": () =>
                fake_window.navigator.userAgent,
              "window.localStorage.setItem": (key: string, value: string) =>
                fake_window.localStorage.setItem(key, value),
            };
            ...
            if (match) {
              ...
              if (funcKey && spoofFunctions[funcKey]) {
                ...
                console.error(
                  `Function for ${funcKey} not found in spoofFunctions.`,
                );
              }
            } else {
